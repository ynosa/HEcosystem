...
//            });
    ...
    var flatten = function (root) {
        ...
        function recurse(node, recurceName) {
            if (node.children) node.children.forEach(function(child){recurse(child, recurceName + node.name + ".")});
            if (!node.id) node.id = recurceName + node.name;
            nodes.push(node);
        }
